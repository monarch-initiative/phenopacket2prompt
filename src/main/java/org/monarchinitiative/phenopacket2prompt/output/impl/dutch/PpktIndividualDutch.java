...
public class PpktIndividualDutch implements PPKtIndividualInfoGenerator {
    ...
    public PpktIndividualDutch() {
        buildBlocks = new DutchBuildingBlocks();
    }
    ...
    public String getIndividualDescription(PpktIndividual individual) {
        ...
        if (onsetOpt.isPresent()) {
            ...
            onsetDescription = "De leeftijd bij het begin van de ziekte werd niet gespecificeerd";
        }
    ...
    private String iso8601onsetDescription(Iso8601Age isoAge) {
        return String.format("De ziekte begon toen de proband %s was",
    ...
    private String iso8601AtAgeOf(Iso8601Age isoAge) {
        ...
        if (components.isEmpty()) {
            ...
            return buildBlocks.atTheAgeOf() + " " + components.get(0) + " en " + components.get(1);
        } else {
    ...
    private String iso8601individualDescription(PhenopacketSex psex, Iso8601Age iso8601Age) {
        ...
        if (y > 17) {
            ...
            return switch (psex) {
                case FEMALE -> String.format("%s %se %s", buildBlocks.probandWasA(), age, buildBlocks.woman());
                case MALE -> String.format("%s %se %s", buildBlocks.probandWasA(), age, buildBlocks.man());
            ...
            return switch (psex) {
                ...
                case MALE -> String.format("%s %se %s", buildBlocks.probandWasA(), age, buildBlocks.adolescentBoy());
    ...
    public String atAgeForVignette(PhenopacketAge ppktAge) {
        if (ppktAge.ageType().equals(PhenopacketAgeType.ISO8601_AGE_TYPE)) {
            return "Op een leeftijd van " + buildBlocks.fromIso((Iso8601Age)ppktAge);
